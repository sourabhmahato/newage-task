...
export default function CustomizedTables({rows, handleRevert}) {
  const classes = useStyles();

  const handleRevertClick = row => {
    handleRevert(row)
  }
        ...
        <TableBody>
          {rows?.map((row) => (
            <StyledTableRow key={row.id}>
              ...
              <Button onClick={() => handleRevertClick(row)} className={classes.btn} size="small" variant="outlined">Revert</Button>
              </StyledTableCell>
            </StyledTableRow>
          ))}
        </TableBody>
